...
if __name__ == '__main__':
    ...
    try: 
        ...
        os.mkdir(savePath)
        try:
            runCreateIndexForDownloadExtension = runScriptThread(savePath, CreateLicenseIndex.CreateIndex, savePath, options.productName)
            runCreateLicenseIndex = runScriptThread(savePath, CreateIndexForDownloadExtension.createIndex, options.productName)
        ...
        print("Need permission to create folder in "+options.savedir+" or the destination already contains a folder named '"+STATS_ZIP+"'")     
